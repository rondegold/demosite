	...
	mdataTags = picXmlDoc.getElementsByTagName("mdata");
	  for (i = 0; i < mdataTags.length; i++) {
		  if(mdataTags[i].getAttribute("tag") == mTagName){ //xxx
			  var j, marticleTags, mcaptionAttr, mimgAttr,containerId, bottomTx,bottomTxId,butText,mcaptionId,mimageId, idprefx;
			  ...
			  for (j = 0; j < marticleTags.length; j++) {
				  ...
				  butText = marticleTags[j].getAttribute("price");
				  ...
				  picHTML += "<div class='mbottomtext' id='"+bottomTxId+"'>"+bottomTx+"</div>"
				  picHTML += "<button>"+butText+"</button>"+"</div>";
